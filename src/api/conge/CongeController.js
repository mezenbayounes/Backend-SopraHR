...
export const CreateConge = async (req, result) => {
  ...
  try {
    ...
    const res = await pool.query(
      ...
      "INSERT INTO conge (user_id, cause, date_debut, sc_debut, date_fin, sc_fin, type_conge) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id,user_id",
...
const notificationContent = `${userName} has submitted a leave request`;




const notificationType = "Leave request"; // Custom type for congé validation notifications
...
export const validateConge = async (req, result) => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      RETURNING id,date_debut,user_id
    ...
    if(etat == "VALIDE"){response="approved";}




  else if (etat == "INVALIDE"){response="rejected";}




    const notificationContent = `Your Leave request with date ${formattedDate} has been ${response}`;
