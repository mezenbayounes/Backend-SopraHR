...
export const updateUserr = async (req, res) => {
    try {
        ...
        if (!req.body) {
            console.error('req.body is undefined. Check if express.json() middleware is configured correctly.');
        ...
        const query = `
            ...
            RETURNING *;
        `;
        const updateResult = await pool.query(query, [username, email, role, hashedPassword, image_url, id]);
...
export async function updateUserDetails(id, username, email, role, hashedPassword, imageUrl) {
    const query = `
        UPDATE users
        SET username = $1, email = $2, role = $3, password = $4, image_url = $5
        WHERE id = $6
