...
export const signup = async (req, res) => {
  ...
  try {
    ...
    // Handle known errors like database or validation issues
...
export const getManagerByUserId = async (userId) => {
  ...
  try {
    ...
    // If no result or id_manager is found, return 7000

    if (result.rows.length === 0 || result.rows[0].id_manager === undefined) {
    ...
    // Return the manager ID if found

    return result.rows[0].id_manager;

  } catch (error) {

    console.error('Error fetching manager:', error);

    throw new Error('An error occurred while fetching the manager');
...
export const getUserNameById = async (userId) => {
  ...
  try {
    ...
    throw new Error('An error occurred while fetching the username');
...
export const getUserNameByIdd = async (req, res) => {
