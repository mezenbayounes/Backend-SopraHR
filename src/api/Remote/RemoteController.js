...
import { getManagerByUserId,getUserNameById } from "../User/authController.js"; // Adjust the path as necessary
...
WHERE id = $1;
    ...
    const managerID= await getManagerByUserId(userId);
    ...
    console.log("manager id of the user :",managerID);
...
const notificationContent = `${userName} has submitted a remote request`;




const notificationType = "Remote request"; // Custom type for congé validation notifications
  ...
  export const RefuseRemoteRequest = async (req, result) => {
    ...
    try {
      ...
      const updateQuery = `
        ...
        RETURNING id, etat,date_remote,user_id
      ...
      const resData = res.rows[0].date_remote;
      ...
      const notificationContent = `Your remote request with date ${formattedDate} has been rejected`;
...
export const validateRemoteRequest = async (req, res) => {
  ...
  try {
    ...
    const resData = updateResult.rows[0].date_remote;




    const resUserId = updateResult.rows[0].user_id;
    ...
    const notificationContent = `Your remote request with date ${formattedDate} has been approved`;
